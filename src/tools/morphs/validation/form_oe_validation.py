...
schema_head = Any([
	...
	custom_error="invalid Old English form structure. Expected schema of type 'form' or 'multiform'"
)
schema_multiform = Dict({ "main": Schema("form"), "alt": One_Or_More(Schema("form")) })
schema_form = Dict({
	"form": One_Or_More(Any([
			String(),
			Schema("paradigm-oe")
		],
		custom_error="invalid 'form' structure. Expected one-or-list of strings or a schema of type 'paradigm-oe'"
	)),
	"canon": Opt(One_Or_More(Any([
		...
		custom_error="invalid 'canon' structure. Expected one-or-list of strings, or a schema of type 'paradigm-oe' or 'canonset'"
	))),
