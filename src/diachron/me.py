...
def phonemes_from_oe_3(oe_phonemes):
    ...
    def diphthong_breaking(state):
        if state.joined[-1] in ["x", "ɣ"] and state.joined[:-1] in ["æɑ", "æːɑ"]:
            return [Phoneme("ɑ", state.capture[0].stressed), Phoneme("w")]

    # eːɑw iːw → ew ju
    def ew_ju(state):
        if state.joined == "eːɑw":
            return [Phoneme("e", state.capture[0].stressed), Phoneme("w")]
        elif state.joined == "iːw":
            return [Phoneme("j"), Phoneme("u", state.capture[0].stressed)]

    # {ɑːw,ɑːɣ,oːw} → ɔːw 
    def owww(state):
        if state.joined in ["ɑːw", "ɑːɣ", "oːw"]:
            return [Phoneme("ɔː", state.capture[0].stressed), Phoneme("w")]
    ...
    def htow(state):
        if state.joined in ["ɑːht", "oht", "oːht"]:
            return [Phoneme("o", state.capture[0].stressed), Phoneme("w")]
    ...
    def ay_vowel_changes(state):
        if state.current.value == "ɑː":
            ...
            return [Phoneme("i", state.current.stressed)]
    ...
    def back_mid_lengthening(state):
        if state.current.value in ["ɑ", "e", "o"] and state.syllable_data.is_open \
            and (state.syllable_data.next_other_syllable_vowel.value == "iː" or state.syllable_data.next_consonant.value in ["m", "n", "r", "l"]):
    ...
    def reduce_final_unstressed_m(state):
    ...
    def reduce_final_vowel(state):
        if state.current.value in ["e", "ɑ" ,"o"] and state.next == None:
            return [Phoneme("ə", state.current.stressed)]
    ...
    rig.run_capture(reduce_final_unstressed_m, 1)
